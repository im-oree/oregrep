...
#[derive(Args)]
pub struct AiFixArgs {
    file: PathBuf,

    /// What to fix (defaults to "any obvious issues you find")
    #[arg(short = 'i', long)]
    issue: Option<String>,
    ...
    #[arg(long = "max-iters", default_value = "6")]
...
pub fn run(args: AiFixArgs) -> Result<()> {
    ...
    let (provider, model) = if let Some(m) = &args.model {
        ...
        let decision = ai_route(&cfg, "fix", "fix")?;
    ...
    let issue = args.issue.clone().unwrap_or_else(|| "any obvious issues you find (bugs, smells, potential crashes)".to_string());
    ...
    let task = format!(
        ...
        1. Use `ore-cat` to read the file.\n\
        2. Use `ore-backup` to save the current state.\n\
        3. Use `ore-patch` or `ore-replace` to apply your fix.\n\
        4. Use `ore-verify` (or `ore-compile-ts` / `ore-compile-rust`) to check the fix.\n\
        5. If verification fails, use `ore-restore` and try a different fix.\n\
        6. Report what you did (or if you couldn't fix it, say so).\n\n\
        You have {} iterations. Be surgical, don't over-refactor.",
        history_prefix, args.file.display(), issue, args.max_iterations
    );
    ...
    let system = get_prompt("fix").unwrap_or_else(|_| "You are a precise bug fixer.".to_string());
    let tools = builtin_tools();
    let agent_cfg = AgentConfig {
        provider, model: model.clone(),
        max_iterations: args.max_iterations,
        auto_approve_destructive: args.auto,
        task_label: "fix".to_string(),
    };
    ...
    match result {
        Ok((answer, _, _, _, _)) => {
            if let Some(ref sname) = session_name {
                let _ = append(sname, "user", &format!("fix {} — {}", args.file.display(), issue));
